...
tx_injuries <- tx_injuries %>% 
  mutate(injury_year = year(injury_date),
         injury_month = month(injury_date),
...
## Insurance Company and Accidents
...
Firstly, the TDI injury report/data set needs to be combined with insurance policies data set, which is also provided by TDI. Then, a summary table table of accidents occured by insurance agents can be constructed.
...
Table \@ref(tab:accidents-by-insurance) shows that the majority of rides related injuries occurred in amusement parks that were overseen by Hartford Fire Insurance Company. It is still larger compared to the number of injuries from the rest of the table combined.
...
```{r accidents-by-insurance-2}
tx_policies_smry <- tx_policies %>% 
  group_by(agent) %>% 
  tally() %>% 
  arrange(desc(n)) %>% 
  rename(insurance_agent = agent,
         num_operations_maintained = n)

tx_comp_summary2 <- tx_comp %>% 
  group_by(agent) %>%
  filter(agent != "NA") %>% 
  tally() %>% 
  arrange(desc(n)) %>% 
  left_join(tx_policies_smry,
            by = c("agent" = "insurance_agent")) %>% 
  rename(total_accidents = n,
         insurance_agent = agent) %>% 
  mutate(accidents_per_operation = round(total_accidents/num_operations_maintained, digits = 2))
```

```{r test, include=FALSE}
tx_comp %>% group_by(agent) %>%
  filter(agent != "NA") %>% 
  tally() %>% 
  arrange(desc(n)) %>% 
  ggplot(aes(x = n,
...
tx_comp %>% group_by(name_of_operation) %>%
  filter(name_of_operation != "NA") %>% 
  tally() %>% 
  arrange(desc(n)) %>% 
  head(10) %>% 
  kable() %>% 
...
tx_comp %>% 
  filter(name_of_operation != "NA") %>% 
  filter(agent != "NA") %>% 
  group_by(name_of_operation, agent) %>% 
  tally() %>% 
  arrange(desc(n)) %>%
  head(10) %>% 
  kable() %>% 
...
tx_policies %>% 
   group_by(agent, name_of_operation) %>% 
   tally() %>% 
   arrange(desc(n)) %>% 
   kable() %>% 
   kable_styling()
...
```{r bus-type}
safer_parks %>% filter(source == "Texas Dept. of Insurance") %>% 
  group_by(bus_type) %>% 
  summarise(total_injured = sum(num_injured)) %>% 
