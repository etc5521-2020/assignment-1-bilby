...
```{r accidents-seasonal-trends}
tx_season <- tx_injuries %>% 
  filter(injury_year != "NA") %>% 
  filter(injury_month != "NA") %>% 
  group_by(injury_year, injury_month) %>% 
  tally()

tx_season$injury_year <- as.factor(tx_season$injury_year)
tx_season$injury_month <- as.factor(tx_season$injury_month)

tx_season_plot <- tx_season %>% 
