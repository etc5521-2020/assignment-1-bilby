...
If we take gender into account, we can see gender distributions by age. Table \@ref(tab:tx-pm1-gender) shows the top-10 rankings by gender and age. In top-10 list, it seems that more girls get injured than boys in amusement parks in Texas. 
...
safer_parks %>% 
  ...
  kable(caption = "Top 10 most dangerours devices types.  **Interesting thing**",
...
```{r accidents-by-insurance-yearly, fig.width=9, fig.cap= "Yearly Trend of Number of Rides Related Injuries in Texas"}
...
## what is the most frequent words in injury description? And what are the characteristics of body part injured related to age, gender and year?
...
```{r bag, fig.cap="A grouped violin plot for age. The main group is body part injured and the subgroup is gender. People who get head injuries are more likely to be young people. A great proportion of people who get lower limbs injuries are boys."}
...
From the Fig \@ref(fig:bag), we find that people who get head injuries are most likely to be people with age below 20. Besides, the number of boys get lower limbs injuries is significantly larger than the number of girls get lower limbs injuries.

The first finding suggests that either devices that has higher chance to hurt people's head are more popular among young people, or young people are at a higher risk of head injuries in amusement park. Both of the hypotheses need to be examined by stakeholders carefully and then take actions to mitigate them.
...
```{r yb, fig.cap="Grouped line plot of number of injury across 4 years faceted with different body part injuries. The main group is gender. The high number of lower limbs injuries in 2017 and upper limbs injuries in 2015 are mainly related to Skygroup investment LLC and Six Flags.", fig.width=9}
temp %>%
...
In Fig. \@ref(fig:yb) we will not compare the absolute values between gender given we don't know the underlying probability of a visitor is a female, but we can assume the total number of visitors in each year will be not that different. Thus, we will focus on the trend over years.

We can observe the cases in upper limbs in 2015 has a significantly high values in both male and female. This one is interesting and if we further investigate it, we will find that in Tab. \@ref(tab:furtheri), they are mostly caused by a ride called "iFly" operated by Skygroup Investments LLC. Even more interestingly, if we dig into the high number of female lower limbs injuries in 2017 in Tab. \@ref(tab:furtherii), we will see the "Hurricane Harbor" operated by Six Flags which is ranked third in Tab. \@ref(tab:furtheri) appears again. In conclusion, there are potential issues in devices operated by these two companies. 

Other than that, in Fig. \@ref(fig:yb) we can observe a clear declining trend of cases in torso. That is a good sign of protecting people from being serious injured. 
...
filter(temp, injury_year == 2015, body_part == "upper limbs") %>%
  ...
  kable(col.names = c("Name of Operation", "count"), caption = "Name of operation related to lower limbs injuries in 2015.")%>% 
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```
...
filter(temp, injury_year == 2017, body_part == "lower limbs", gender == "F") %>%
  ...
  summarise(count = n()) %>%
  arrange(-count, name_of_operation) %>%
  kable(col.names = c("Name of Operation", "count"), caption = "Name of operation related to female upper limbs injuries in 2015.") %>% 
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```
...
In this report, we analysed the amusement park injuries datasets of the U.S. Since there's no data related to the total number of visitors, it's hard to say whether the parks with less injuries are more safer than others. Keeping this in mind, we find several interesting questions.

- About half of the injured visitors are younger than 35 years old, and the age group with the most injuries is children between 10 and 15 years old. Injuries of girls in this age group is slightly more than that of boys.

- Coasters caused more injuries than other kind of rides. In high proportion of injury cases, the injured get hurt of their upper half of the body. 

- In summer, there are more visitors injured in amusement parks, while in winter the number of injuries is much smaller. This may because there are more visitors in summer.

- Of all the injuries, the most are related to the devices insured by the Hartford Fire Insurance Company. But having a high number of injuries is not necessarily a reflection of an insurance company being negligence. It may be the case that the parks overseen by the insurance company attracts much more guests compared to other parks. 

- In Texas, the Six Flags Over Texas park has the most amusement park injuries, especially in year 2013 and 2014. The Skygroup Investments LLC DBA iFly Houston Memorial park had a significant large number of injuries in 2015, while the Typhoon Texas Austin Park had lots of injuries in 2016.

- The in-house manufactured amusement park devices accounts for more than 10% of total injuries. This is a security hazard that can't be ignored. The top 10 manufacturers who caused most injuries accounted for 32.3% of all injuries, while there are 254 manufacturers’ records in this data set.
