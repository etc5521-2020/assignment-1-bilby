...
Analyse the safer_parks data by state, find the states with the most amusement park injuries from 1/1/2011 to 9/9/2016.

```{r state-city-injuries}
safer_parks %>% 
  group_by(acc_city, acc_state) %>% 
  summarise(total_injuries = sum(num_injured)) %>% 
  arrange(desc(total_injuries)) %>% 
  head(10) %>% 
  kable(caption = "Cities with the most amusement park injuries", booktabs = T) %>% 
...
```{r manufacturer}
safer_parks %>% 
  group_by(manufacturer, acc_state) %>% 
  summarise(total_injuries_manu = sum(num_injured)) %>% 
  arrange(desc(total_injuries_manu))
```
